...
public class OpponentShip extends Ship {
	...
	/**
	 * Creates a Ship for the opponent at the provided position.
	 * @param isPlayer whether the ship is for the player
	 * @param position the location to create it
	 */
	public OpponentShip(boolean isPlayer, Vector2<Double> position) {
		// Creates the ship
		...
		weaponControl = new WeaponControl(new Vector2<Double>(0.08, 0.02), getNextTilePosition(), isPlayer, 0, 3);
		addTile(weaponControl);
		
		motor = new Motor(new Vector2<Double>(0.5, 0.02), getNextTilePosition(),isPlayer, 0);
		addTile(motor);
		
		shield = new Shield(new Vector2<Double>(0.58, 0.02), getNextTilePosition(),isPlayer, 0);
		addTile(shield);
